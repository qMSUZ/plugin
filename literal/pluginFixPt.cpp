...
data_t reciprocalFixPt(const data_t &v)
{
    ...
    data_t x = (data_t)((data_t)(48.0/17.0) - (data_t)(39.0/17.0) * D);
...
data_t lnFixPt(const data_t &v)
{
  ...
  for ( lnFP_i = 0; lnFP_i < n; lnFP_i++ )
  {
    ...
    {
    	lnFP_ai = (data_t)1.0 + ( lnFP_ai - (data_t)1.0 ) / (data_t)2.0;
    }

    if ( lnFP_ai < lnFP_x )
    {
      lnFP_w[ lnFP_i ] = (data_t)1.0;
      lnFP_x = lnFP_x / lnFP_ai;
    }
  }

  lnFP_x = lnFP_x - (data_t)1.0;


  lnFP_x = lnFP_x
    * ( (data_t)1.0 - ( lnFP_x / (data_t)2.0 )
    * ( (data_t)1.0 + ( lnFP_x / (data_t)3.0 )
    * ( (data_t)1.0 -   lnFP_x / (data_t)4.0 )));
  ...
  for ( lnFP_i = 0; lnFP_i < n; lnFP_i++ )
  {
    lnFP_x = lnFP_x + lnFP_w[ lnFP_i ] * lnFP_poweroftwo;
    lnFP_poweroftwo = lnFP_poweroftwo / (data_t)2.0;
  }

  lnFP_fx = lnFP_x + ( data_t )lnFP_k;
...
data_t expFixPt ( const data_t &x )
{
  ...
  for ( expFP_i = 0; expFP_i < n; expFP_i++ )
  {
    ...
    expFP_poweroftwo = expFP_poweroftwo / (data_t)2.0;
  }
  ...
  for ( expFP_i = 0; expFP_i < n; expFP_i++ )
  {
    ...
    {
      expFP_ai = (data_t)1.0 + ( expFP_ai - (data_t)1.0 ) / (data_t)2.0;
    }

    if ( (data_t)0.0 < expFP_w[ expFP_i ] )
    {
      expFP_fx = expFP_fx * expFP_ai;
    }
  }

  expFP_fx = expFP_fx
    * ( (data_t)1.0 + expFP_z
    * ( (data_t)1.0 + expFP_z / (data_t)2.0
    * ( (data_t)1.0 + expFP_z / (data_t)3.0
    * ( (data_t)1.0 + expFP_z / (data_t)4.0 ))));

  if ( expFP_x_int < 0 )
  {
    for ( expFP_i = 1; expFP_i <= -expFP_x_int; expFP_i++ )
    {
      expFP_fx = (data_t)(expFP_fx * einv);
    }
  }
  else
  {
...
data_t varianceFixPt( const data_t &v, const uint8bit_t &op )
{
    ...
    if( op == varest_col_op)
    {
        ...
        tt1 = v + v;
        varFP_t2 = varFP_t2 + tt1;
    }

    if( op == varest_fin_op)
    {
         tt1 = v - (data_t)1.0;
         varFP_tmp1 =  (data_t)1.0 / (v * tt1);
         varFP_tmp2 = (data_t)1.0 /  tt1 ;

         tt1 = varFP_t1 * varFP_t1;
         varFP_Vt =  varFP_tmp1 * tt1;
         tt1 = varFP_tmp2 * varFP_t2;
         varFP_V = tt1 - varFP_Vt;
...
data_t estimate_g1_FixPt(const data_t &t, const data_t &_N )
{
    ...
    estg1FP_x = (data_t)((data_t)(11.968268412042981) * (data_t)1.0 / ( ( mi2k * (t * _N) ) ));
...
data_t fnc_k6x_FixPt( const data_t &x )
{
    ...
    tmp1 = -(k6xFP_x2 >> 1);
    tmp1 = expFixPt( tmp1 );
    tmp2 = ((data_t)-15 + k6xFP_x2);
    tmp2 = (k6xFP_x2 * tmp2);
    k6xFP_t2 = (tmp1 * ((data_t)-15.0 + (k6xFP_x2 * ((data_t)45.0 + tmp2))) );


    return (data_t)( t1 * k6xFP_t2);
}
...
data_t estimate_psi6_FixPt( const data_t &v1, const data_t &v2, const uint8bit_t &op )
{
    ...
    if( op == estpsi6_setup_op )
    {
        ...
        for(i=0;i<6;i++)
        {
                psi6_FP_g7 = (psi6_FP_g7 * psi6_FP_g1);
        }


        psi6_FP_g1 = (data_t)1.0 / v1;

        tmp1 = (psi6_FP__N * psi6_FP__N);
        tmp2 = (tmp1 * psi6_FP_g7);
        psi6_FP_t = (data_t)1.0 / tmp2;
    ...
    if( op == estpsi6_col_op_new )
    {
        psi6_f1: for( i=0; i<totalDataLen; i ++)
        {
            ...
            {
                tmp1 = (wbuf[i] -wbuf[j]);
                psi6_FP_tmpv = (tmp1 * psi6_FP_g1);
                psi6_FP_s = (psi6_FP_s + fnc_k6x_FixPt( psi6_FP_tmpv ));
            }
        }
    }
    ...
    {
        psi6_FP_tmpv = (psi6_FP__N * K6zero);
        psi6_FP_rslt = (psi6_FP_t * ( ((data_t)2.0 * psi6_FP_s) + psi6_FP_tmpv ));
    }
...
data_t estimate_g2_FixPt( const data_t &psi6g1, const data_t &_N )
{
    ...
    g2 = (data_t)powerxyFixPt( (data_t) ((data_t)((data_t)-2.0 * k40) / ( (data_t)((data_t)mi2k * (data_t)psi6g1 * (data_t)_N)) ), (data_t)((data_t)1.0/(data_t)7.0) );
...
data_t fnc_k4x_FixPt(const data_t &x )
{
    ...
    k4xFP_x2 = (x * x);
    k4xFP_x4 = (k4xFP_x2 * k4xFP_x2);

    return ( (data_t)(t1 * (data_t)((data_t)k4xFP_x4 - (data_t)6.0*k4xFP_x2 + (data_t)3.0) * expFixPt( (data_t)((data_t)-0.5 * (data_t)k4xFP_x2)) ));
}
...
data_t estimate_psi4g2_FixPt( const data_t &v1, const data_t &v2, const uint8bit_t &op )
{
    ...
    if ( op == estpsi4_setup_op )
    {
        ...
        for(i=0;i<4;i++)
        {
            psi4g2_FP_gg5 = (psi4g2_FP_gg5 * v1);  // ^2
        }


        psi4g2_FP_t = ((data_t)1.0 / (data_t)( (data_t)(psi4g2_FP__N * psi4g2_FP__N) * (data_t)psi4g2_FP_gg5)) ;
        psi4g2_FP_s = (data_t)0.0;

        psi4g2_FP_g2 = (data_t)1.0 / v1 ;
    ...
    if ( op == estpsi4_col_op_new )
    {
    		psi4_f1: for( i=0; i<totalDataLen; i++ )
            {
                ...
                {
                    psi4g2_FP_s = (psi4g2_FP_s + fnc_k4x_FixPt( (data_t)(( wbuf[i] - wbuf[j] ) * psi4g2_FP_g2) ) );
                }
            }
    }
    ...
    {
            psi4g2_FP_rslt = (psi4g2_FP_t * ( ((data_t)2.0 * psi4g2_FP_s) + (psi4g2_FP__N * K4zero) ));
    }
...
data_t estimate_h_FixPt( const data_t &psi4g2, const data_t &_N )
{
    ...
    return powerxyFixPt( (data_t)(((data_t)(rk)) / ( (data_t)((data_t)(mi2k * psi4g2) * _N) )), (data_t)((data_t)1.0/(data_t)5.0) );
